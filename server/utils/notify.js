...
// Create a personal notification for a user + push it live (+ optional email).
// Fire-and-forget: never breaks the main request.
// Usage: notify({ orgId, userId, type, message, link, email: true })
const notify = ({ orgId, userId, type, message, link = '', email = false }) => {
  ...
  // Optional email (only if asked). Look up the user's email, then send.
  if (email) {
