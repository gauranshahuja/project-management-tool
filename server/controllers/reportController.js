...
// Only managers can pull org-wide reports
const isManager = (user) => ['Owner', 'Admin', 'Manager'].includes(user.role);

// Build a createdAt date filter from ?from=YYYY-MM-DD&to=YYYY-MM-DD
...
// @route GET /api/reports/orders?from=&to=&format=csv   (Owner/Admin/Manager)
...
// @route GET /api/reports/stock    (current stock snapshot, not date-filtered)
...
// @route GET /api/reports/ledger?from=&to=
...
// @route GET /api/reports/attendance?from=&to=
exports.attendanceReport = asyncHandler(async (req, res) => {
  if (!guardManager(req, res)) return;
  // Attendance.date is a "YYYY-MM-DD" string — filter on that
...
// @route GET /api/reports/tasks?from=&to=   (PM side — all org tasks)
