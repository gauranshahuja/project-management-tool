...
// { productId, locationId, qty, orderId?, batchNo?, reason?, disposition? }
exports.createReturn = asyncHandler(async (req, res) => {
  ...
  if (orderId) {
    ...
    if (!batchNo && order.batchBreakdown?.length) {
      batchNo = order.batchBreakdown[0].batchNo; // default to first batch of the order
    }
  }
  if (!batchNo) batchNo = 'RETURN';

  // Restock only if not damaged
  if (disposition === 'Restocked') {
    ...
    // Damaged: record movement without adding to sellable stock
    writeLedger(req.user, {
      reason: 'return',
      product: productId,
      sku: product.sku,
...
// @route GET /api/inventory/returns?locationId=&limit=
...
// Dashboard summary: orders/returns counts + units, by status.
