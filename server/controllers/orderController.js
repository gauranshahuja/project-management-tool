...
// Per-org incrementing number: count existing + 1, zero-padded with a prefix.
const nextNumber = async (Model, orgId, prefix) => {
  const count = await Model.countDocuments({ organization: orgId });
  return `${prefix}-${String(count + 1).padStart(5, '0')}`;
};

// FEFO batches for a product at a location (earliest expiry first, null last, qty>0)
...
// @route GET /api/inventory/orders?locationId=&status=&limit=
...
// @route PATCH /api/inventory/orders/:id/status  { status }
exports.updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  // Cancelling a fulfilled order restocks the batches (FEFO breakdown)
...
// { fromLocationId, toLocationId, productId, batchNo?, qty }  (Owner/Admin/Manager)
exports.createTransfer = asyncHandler(async (req, res) => {
  ...
  // Pick source batch: given batchNo, or FEFO earliest with enough qty
  let sourceBatch;
  if (batchNo) {
  ...
  // Ship: deduct from source now (in-transit)
...
// @route GET /api/inventory/transfers?status=&limit=
...
// @route PATCH /api/inventory/transfers/:id/accept  (Owner/Admin/Manager)
